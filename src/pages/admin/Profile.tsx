...
const AdminProfile = () => {
  ...
  useEffect(() => {
    ...
    getMyProfile()
      ...
      .catch((err) => {
        if (!ignore) setError(getApiErrorMessage(err, 'Không tải được hồ sơ admin.'));
      })
      .finally(() => {
  ...
  const uploadAvatar = async (file: File) => {
    ...
    try {
      ...
      await refreshMe();
      setSuccess('Ảnh đại diện admin đã được cập nhật.');
    } catch (err) {
  ...
  return (
    <>
      <Topbar title="Hồ sơ admin" subtitle="Thông tin tài khoản quản trị đang đăng nhập vào hệ thống Happabi." />
      ...
      {isLoading ? (
        <Card className="flex min-h-[420px] items-center justify-center">
          <p className="text-[14px] font-bold text-text-mid">Đang tải hồ sơ admin...</p>
        </Card>
      ) : (
                ...
                <div className="flex items-center gap-5">
                  <div className="relative">
                    ...
                    <button
                      ...
                      aria-label="Cập nhật avatar admin"
                    >
                      <Camera size={16} />
                    </button>
                    <input
                  ...
                  <div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {(profile?.roles || []).map((role) => (
                        <Tag key={role} variant={role === 'ADMIN' ? 'grad' : 'purple'}>
                          {roleLabel[role] || role}
                        </Tag>
                      ))}
                    </div>
          ...
          <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
            ...
            <Card>
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-2xl border border-lav-100 bg-white p-4">
                  <div className="flex items-center gap-3">
                    <Clock size={18} className="text-lav-dark" />
                    <div>
                      <p className="text-[12px] font-bold text-text-light">Ghi chú vận hành</p>
                      <p className="mt-1 text-[13px] font-bold leading-5 text-text-mid">
                        Các thay đổi role, trạng thái user và audit log được thực hiện ở các màn hình quản trị riêng.
                      </p>
                    </div>
                  </div>
                </div>
