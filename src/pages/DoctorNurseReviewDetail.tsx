      ...
      {isLoading ? (
        ...
        <Card className="flex min-h-[520px] items-center justify-center text-center">
          <div>
            ...
            <div className="font-semibold text-text-dark">Không tìm thấy hồ sơ nurse</div>
  ...
  <div className="border-b border-lav-100 bg-[#fff9fb] px-6 py-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <h1 className="mt-3 text-2xl font-semibold text-text-dark">{selected.fullName || 'Nurse Happabi'}</h1>
...
const Section = ({ title, icon, children }: { title: string; icon: ReactNode; children: ReactNode }) => (
  <section className="rounded-2xl border border-lav-100 bg-white p-5">
    <div className="mb-4 flex items-center gap-2 font-semibold text-text-dark">
...
const Metric = ({ label, value }: { label: string; value: string }) => (
  <div className="rounded-2xl border border-lav-100 bg-white px-4 py-3 text-right">
    <div className="text-[11px] font-semibold uppercase tracking-wide text-text-light">{label}</div>
    <div className="mt-1 text-xl font-semibold text-lav-dark">{value}</div>
  </div>
);
...
const CertificationCard = ({ cert, loading, onOpen }: { cert: NurseCertification; loading: boolean; onOpen: () => void }) => (
  <div className="rounded-2xl border border-lav-100 bg-[#fff9fb] p-4">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div>
        <div className="font-semibold text-text-dark">{cert.certName || 'Chứng chỉ nurse'}</div>
