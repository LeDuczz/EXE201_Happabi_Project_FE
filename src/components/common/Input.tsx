...
const Input: React.FC<InputProps> = ({ label, icon, hint, className = '', ...props }) => {
  return (
    <div className="mb-4">
      {label && (
        <div className="text-caption font-medium text-text-mid mb-1.5">
          {label}
        </div>
      )}
      <div className="relative">
        ...
        <input
          className={`
            w-full rounded-xl border border-lav-200 bg-white text-body font-normal text-text-dark outline-none transition-colors duration-200
      ...
      {hint && <div className="text-caption text-text-light mt-1">{hint}</div>}
    </div>
  );
};
